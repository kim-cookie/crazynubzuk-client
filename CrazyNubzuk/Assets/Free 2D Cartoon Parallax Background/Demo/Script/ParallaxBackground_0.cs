...
public class ParallaxBackground_0 : MonoBehaviour
{
    ...
    void Update(){
        ...
        if (Camera_Move)
        {
            Vector3 moveDelta = Vector3.right * Time.deltaTime * Camera_MoveSpeed;
            _camera.position += moveDelta;
            if (nubzuk != null) nubzuk.transform.position += moveDelta;
            if (ground != null) ground.transform.position += moveDelta;
        }
        for (int i=0;i<5;i++)
        {
