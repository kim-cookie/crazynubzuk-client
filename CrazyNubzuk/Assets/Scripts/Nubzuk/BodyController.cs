// using UnityEngine;

// public class BodyController : MonoBehaviour
// {
//     [Header("물리 설정")]
//     public Rigidbody2D bodyRb;
//     public float torquePower = 1f;
//     public float stabilityTorque = 0.1f;
//     public float maxAngularVelocity = 10f;

//     private string myDirection;

//     void Start()
//     {
//         float randomTorque = Random.Range(-1f, 1f);
//         bodyRb.AddTorque(randomTorque, ForceMode2D.Impulse);
//         // 본인의 방향 저장
//         // myDirection = UserDataManager.Instance.direction.ToLower(); // "left" or "right"
//     }

//     void Update()
//     {
//         if (Input.GetKey(KeyCode.LeftArrow))
//         {
//             bodyRb.AddTorque(torquePower);
//         }
//         else if (Input.GetKey(KeyCode.RightArrow))
//         {
//             bodyRb.AddTorque(-torquePower);
//         }
//         // // Left 방향을 맡은 경우
//         // if (myDirection == "left" && Input.GetKey(KeyCode.LeftArrow))
//         // {
//         //     bodyRb.AddTorque(torquePower);
//         // }
//         // // Right 방향을 맡은 경우
//         // else if (myDirection == "right" && Input.GetKey(KeyCode.RightArrow))
//         // {
//         //     bodyRb.AddTorque(-torquePower);
//         // }
//     }

//     void FixedUpdate()
//     {
//         float zAngle = NormalizeAngle(transform.eulerAngles.z);

//         // 기울어진 각도에 따라 torque 가중 (중심을 기준으로 무너지는 듯)
//         float torque = zAngle * stabilityTorque;

//         // Z 축 기준 Torque 적용 (양수면 시계방향 회전)
//         bodyRb.AddTorque(torque);

//         // 최대 회전 속도 제한 (선택사항)
//         bodyRb.angularVelocity = Mathf.Clamp(bodyRb.angularVelocity, -maxAngularVelocity, maxAngularVelocity);
//     }

//     // 회전각을 -180 ~ +180도로 정규화
//     float NormalizeAngle(float angle)
//     {
//         angle = angle % 360f;
//         if (angle > 180f) angle -= 360f;
//         return angle;
//     }
// }


using UnityEngine;

public class BodyController : MonoBehaviour
{
    [Header("물리 설정")]
    public Rigidbody2D bodyRb;
    public float smoothSpeed = 5f;           // 회전 보간 속도

    private float targetZRotation = 0f;
    private float currentZRotation = 0f;
    
    private bool isLeftPressed = false;
    private bool isRightPressed = false;

    private string myNickname;
    private string myDirection;

    void Start()
    {
        currentZRotation = 0f;
        transform.rotation = Quaternion.Euler(0f, 0f, currentZRotation);

        myNickname = UserDataManager.Instance.nickname;
        myDirection = UserDataManager.Instance.direction.ToLower(); // "left" or "right"
    }

    void Update()
    {
        HandleInput();

        // 부드럽게 현재 회전 보간
        currentZRotation = Mathf.Lerp(currentZRotation, targetZRotation, Time.deltaTime * smoothSpeed);
        transform.rotation = Quaternion.Euler(0f, 0f, currentZRotation);
    }

    /// 서버에서 받은 balance 값을 기반으로 회전 적용
    public void ApplyBalanceRotation(float balance)
    {
        balance = Mathf.Clamp(balance, -100f, 100f);
        targetZRotation = -balance * 1.8f;  // -100~100 → 180~-180
    }

    /// 다리 회전에 사용될 현재 회전값 반환
    public float GetCurrentZRotation()
    {
        return currentZRotation;
    }

    private void HandleInput()
    {
        // 왼쪽 방향을 맡은 유저만 왼쪽 입력 가능
        if (myDirection == "left")
        {
            if (Input.GetKeyDown(KeyCode.LeftArrow) && !isLeftPressed)
            {
                isLeftPressed = true;
                NetworkManager.Instance.Send(new RequestPacketData.LeftIn(myNickname));
            }
            else if (Input.GetKeyUp(KeyCode.LeftArrow) && isLeftPressed)
            {
                isLeftPressed = false;
                NetworkManager.Instance.Send(new RequestPacketData.LeftOut(myNickname));
            }
        }

        // 오른쪽 방향을 맡은 유저만 오른쪽 입력 가능
        else if (myDirection == "right")
        {
            if (Input.GetKeyDown(KeyCode.RightArrow) && !isRightPressed)
            {
                isRightPressed = true;
                NetworkManager.Instance.Send(new RequestPacketData.RightIn(myNickname));
            }
            else if (Input.GetKeyUp(KeyCode.RightArrow) && isRightPressed)
            {
                isRightPressed = false;
                NetworkManager.Instance.Send(new RequestPacketData.RightOut(myNickname));
            }
        }
    }
}
