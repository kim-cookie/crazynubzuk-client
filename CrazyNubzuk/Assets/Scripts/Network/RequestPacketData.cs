...
public abstract record RequestPacketData
{
    ...
    // 로비 입장
    public sealed record EnterLobby(string nickname) : RequestPacketData;
    // 방 생성
    public sealed record CreateRoom(string nickname, string roomId) : RequestPacketData;
    // 방 입장
    public sealed record EnterRoom(string nickname) : RequestPacketData;
    // 준비
    public sealed record ReadyGame(string nickname) : RequestPacketData;
    // 게임 시작
    public sealed record StartGame(string nickname, string role) : RequestPacketData;
    // 오른쪽 입력
    public sealed record RightIn(string nickname) : RequestPacketData;
    // 오른쪽 입력 해제
    public sealed record RightOut(string nickname) : RequestPacketData;
    // 왼쪽 입력
    public sealed record LeftIn(string nickname) : RequestPacketData;
    // 왼쪽 입력 해제
    public sealed record LeftOut(string nickname) : RequestPacketData;
    // 재시작
    public sealed record Restart(string nickname) : RequestPacketData;
    // 나가기 (로비)
    public sealed record Exit() : RequestPacketData;
    // 기록
    public sealed record Record(string roomId) : RequestPacketData;
}
