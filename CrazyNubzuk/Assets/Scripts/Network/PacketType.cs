public static class PACKET_TYPE
{
    ...
    public const int PONG = 2; // 보통 PING과 PONG signal이 동일하면 안 되지만, 위 딕셔너리를 기준으로 일치시킴

    // Lobby & Room
    public const int ENTER_LOBBY = 1001;
    public const int CREATE_ROOM = 1002;
    public const int ENTER_ROOM = 1003;

    // Game Ready & Start
    public const int READY_GAME = 1011;
    public const int START_GAME = 1012;

    // Move Events
    public const int RIGHT_IN = 1013;
    public const int RIGHT_OUT = 1014;
    public const int LEFT_IN = 1015;
    public const int LEFT_OUT = 1016;

    // Restart & Exit
    public const int RESTART = 1017;
    public const int EXIT = 1018;

    // Game Records
    public const int RECORD = 1021;

    // Game Balance & Results
    public const int UPDATE_BALANCE = 1031;

    // 추가 확장 시 여기에 계속 붙이면 됨
}
